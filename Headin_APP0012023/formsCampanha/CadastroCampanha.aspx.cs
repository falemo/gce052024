...
{
    public partial class CadastroCampanha : System.Web.UI.Page
    {
        ...
        private void InserirRegistro()
        {
            try
            {
                if (new CampanhaBL().InserirCampanha(decimal.Parse(txtVlrCampanha.Text), DateTime.Parse(txtDtInicio.Text), DateTime.Parse(txtDtFim.Text), chkFlAtiva.Checked, txtDsPix.Text, txtDsPixInfo.Text, ((Sessions)Session["pessoa"]).UserId.Id, txtFilePath.Text, decimal.Parse(txtVlrMinimoSorte.Text)) > 0)
                {
                    LimparCampos();
